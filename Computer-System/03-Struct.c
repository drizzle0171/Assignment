...
Node *inputpoly(void){
    struct Node *first; // 가장 첫 번째 poly를 가리킬 구조체 포인터
    struct Node *last; // 이전에 입력한 poly를 기억할 구조체 포인터
    struct Node node; // 입력받는 poly를 저장할 구조체
    struct Node *pt; // 현재 입력 받는 poly 구조체를 가리킬 구조체 포인터
    ...
    while(1) { // 계속해서 입력을 받아야 하므로 무한 루프로 설정
        float deg, coef; // 현재 입력 받는 두 값을 담아둘 실수 변수 선언 (deg = 차수, coef = 계수)
        ...
        else if ((deg >= 0) && coef > 0) { // 만약 deg와 coef 모두 양의 정수라면 else if 문 실행
                if (first == NULL){ // first가 NULL일 때
                    first = pt; // 지금까지 받은 deg와 coef의 정보가 담긴 구조체를 가리키는 pt를 first와 동일시함
                    ...
                    if (last == NULL){ // last, 즉 마지막 node를 가리키는 last 포인터가 NULL일 때 (여기서는 두 번째 node를 가리킬 포인터가 됨)
                        first -> next = pt; // first의 next는 현재 받은 pt이므로 이를 저장
                        pt -> prev = first; // pt, 즉 현재 받은 node의 prev는 first이므로 이를 저장

                    }
                    else { // 세번째 노드부터 실행
                        pt -> prev = last; // pt(세번째 노드라고 가정 했을 때)의 이전은 last(last는 지금 두 번째 노드를 가리키고 있음)임
                        last -> next = pt; // last(두 번째 노드를 가리키고 있음)의 next는 pt(세 번째 노드를 가리키고 있음)임
                    };
               }
